...
fn dependencies_become_ready_at_candidate_and_context_recovers_receipts() {
    ...
    assert!(service.store.ready(&service.store.get("task://down").unwrap()).unwrap());
...
fn dependency_sync_requires_ready_branch_and_stale_workspace_is_rebuilt() {
    ...
    service.store.execution("task://up", "running", "candidate", 1, None, None).unwrap();
    service.store.workspace("task://up", "candidate", None, None).unwrap();
    service
        .call("task.claim", &json!({"owner":"b","filter":{"op":"eq","path":"uri","value":"task://down"}}))
        .unwrap();
    assert!(
...
fn more_fail_closed_and_destroy_paths_raise_coverage() {
    ...
    service
        ...
        .unwrap();
    service.call("workspace.prepare", &json!({"task":"task://up"})).unwrap();
    service.store.execution("task://down", "backlog", "running", 0, Some("b"), None).unwrap();
    assert!(
        service
            .call("workspace.prepare", &json!({"task":"task://down"}))
            .unwrap_err()
    ...
    service
        ...
        .unwrap();
    service.store.execution("task://live", "running", "backlog", 0, None, None).unwrap();
    assert!(
        service
...
"#,
    ...
    let two = service.store.get("task://two").unwrap();
    service
        .store
        .workspace("task://two", &two.state, two.branch.as_deref(), Some(leftover.to_str().unwrap()))
        .unwrap();
