...
impl Store {
    ...
    pub fn claim(&mut self, uris: &[String], owner: &str, lease_until: i64, workflows: &[Option<String>]) -> Result<Vec<TaskRow>> {
        ...
        for (uri, workflow) in uris.iter().zip(workflows) {
            let changed = tx.execute(
                "UPDATE task SET state='running', owner=?2, lease_until=?3, workflow=coalesce(workflow,?4), error=NULL, revision=revision+1 WHERE uri=?1 AND state='backlog'",
                params![uri, owner, lease_until, workflow],
            )?;
            if changed != 1 {
    ...
    pub fn execution(&self, uri: &str, state: &str, step: usize, owner: Option<&str>, error: Option<&str>) -> Result<()> {
        if self.0.execute(
            "UPDATE task SET state=?2,step=?3,owner=?4,lease_until=CASE WHEN ?4 IS NULL THEN NULL ELSE lease_until END,error=?5,revision=revision+1 WHERE uri=?1",
            params![uri, state, step as i64, owner, error],
        )? != 1
        {
            bail!("task not found: {uri}");
        }
        Ok(())
    }
