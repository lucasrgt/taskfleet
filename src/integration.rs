...
pub fn run(config: &Config, store: &Store, filter: &Filter, input: &Value) -> Result<Value> {
    ...
    while !pending.is_empty() {
        ...
        let Some(card_branch) = row.branch.as_deref() else {
            store.execution(&row.task.uri, "blocked", row.step, None, Some("missing task branch"))?;
            blocked.push(row.task.uri);
            continue;
        };
        if runtime::git(&path, &["merge", "--no-ff", "--no-commit", card_branch]).is_err() {
            let _ = runtime::git(&path, &["merge", "--abort"]);
            store.execution(&row.task.uri, "blocked", row.step, None, Some("merge conflict"))?;
            blocked.push(row.task.uri);
            continue;
        }
        let gates = config
        ...
        if green {
            ...
            store.execution(&row.task.uri, "done", row.step, None, None)?;
            merged.push(row.task.uri);
        } else {
            runtime::git(&path, &["merge", "--abort"])?;
            store.execution(&row.task.uri, "blocked", row.step, None, Some("integration gate failed"))?;
            blocked.push(row.task.uri);
        }
    }
